...
(function($) {
	...
	$.fn.seatCharts = function (setup) {
				...
				return function (setup) {
					...
					fn.settings.$node
						.attr({
							...
							title: (fn.settings.id).split('__').pop(),
							'data-tooltip': 'tooltip'
						})
						.text((fn.settings.id).split('__').pop())
		...
		$.each(settings.map, function(row, characters) {
			...
			if (settings.naming.left && !settings.naming.rows[row]=='') {
				$row.append(
					$('<div></div>')
						.addClass('seatCharts-cell seatCharts-space seatCharts-colsname')
						.text(settings.naming.rows[row])
				);
				$row.prepend(
					$('<div></div>')
						.addClass('seatCharts-cell seatCharts-space seatCharts-colsname--right')
						.text(settings.naming.rows[row])
				);
			}
			...
			$.each(characters.match(/[a-z_]{1}(\[[0-9a-z_]{0,}(,[0-9a-z_ ]+)?\])?/gi), function (column, characterParams) { 
				...
				$row.append(character != '_' ?
					...
					$('<div></div>').addClass('seatCharts-cell seatCharts-space empty--seats')	
				);
			});
			
			fn.append($row);
			if($row.find('.seat--double').length !== 0){
				var sib = $row.closest('.theater__map').find('.seatCharts-row:nth-of-type(4)').width();
				$row.addClass('seat--double--row').css('width',sib);
